...
router.post("/add", auth, async (req, res) => {
    try {
        ...
        const nlpAnalysis = nlpService.analyzeTaskText(title + ' ' + (description || ''));
...
router.post("/process-input", auth, async (req, res) => {
    try {
        ...
        const description = inputText.length > title.length ? inputText.substring(title.length + 1) : '';
        const priority = nlpAnalysis.priority;
        const dueDate = nlpAnalysis.deadline.hasDeadline ? nlpAnalysis.deadline.date : null;
        const category = nlpAnalysis.category;
        const contactPerson = nlpAnalysis.contactPerson;
...
router.put("/update/:id", auth, async (req, res) => {
    try {
        let { title, description, priority, dueDate, status, category, contactPerson } = req.body;
        ...
        if (existingTask && (existingTask.title !== title || existingTask.description !== description)) {
        ...
        const task = await Task.findOneAndUpdate(
            { _id: req.params.id, userId: req.user.id },
            { title, description, priority, dueDate, status, category, contactPerson },
            { new: true } // Return the updated document
        );
